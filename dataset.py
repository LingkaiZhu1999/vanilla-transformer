...
class Multi30kDe2En(Dataset):
    ...
    def __init__(self, split: Literal['train', 'valid', 'test'], istoken=True):
        super(Multi30kDe2En, self).__init__()
        self.split = split
        self.iter = Multi30k(root='./multi30k', split=split, language_pair=('de', 'en'))
    ...
    def _load_vocabs(self):
        data_iter = Multi30k(root='./multi30k', split='train', language_pair=('de', 'en'))  # vocabs must be extracted from train split
    ...
    def collate_fn(cls, batch):
        de_batch, en_batch = [], []
        for de, en in batch:
            de_batch.append(torch.cat([torch.tensor([cls.BOS_IDX]), de, torch.tensor([cls.EOS_IDX])], dim=0)) # <bos> de <eos>
            en_batch.append(torch.cat([torch.tensor([cls.BOS_IDX]), en, torch.tensor([cls.EOS_IDX])], dim=0)) # <bos> en <eos>
...
if __name__ == '__main__':
    ...
    dataset = Multi30kDe2En('train', istoken=False)
